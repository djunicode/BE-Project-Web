...
class SearchProjectView(APIView):
    ...
    def get(self,request):
        ...
        inhouse = Inhouse_Project.objects.filter(title__startswith = request.query_params['name'])
        outhouse = Outhouse_Project.objects.filter(title__startswith = request.query_params['name'])
        ...
        count.append({
            'count':inhouse.count() + outhouse.count()
        })

        inhouse_serializer = InhouseProjectSerializer(inhouse, many = True, context = context)
        outhouse_serializer = OuthouseProjectSerializer(outhouse, many = True, context = context)
