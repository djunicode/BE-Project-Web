...
def change_password(request):
    ...
    if not user:
        return JsonResponse(data={"message": "Wrong password provided"}, status=status.HTTP_400_BAD_REQUEST)
    serializer = ChangePasswordSerializer(data=request.data)
    serializer.is_valid(raise_exception=True)
    request.user.set_password(serializer.validated_data["new_password"])
    request.user.save()
    return JsonResponse(data={"message": "Successfully changed password"}, status=status.HTTP_204_NO_CONTENT)
